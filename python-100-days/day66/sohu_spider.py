...
def start_crawl(seed_url,match_pattern,*,max_depth=1):
    conn=pymysql.connect(host='localhost',port=3306,database='crawler',user='root',
                         password='yourpass',charset='utf8')
    try:
                ...
                if depth!=max_depth:
                    ...
                    for link in links_list:
                        if 'http' not in link[0]:
                            ...
                            param_list.append((link[1],link[0]))
...
def main():
    ...
    start_crawl('http://sports.sohu.com/nba_a.shtml',r'<a.*?href="(.+?)".*?>(.+?)</a>',
                max_depth=2)
