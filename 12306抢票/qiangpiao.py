...
class BrushTicket(object):
    def __init__(self,passengers,from_time,from_station,to_station,my_start_time,my_end_time,seat_type,receiver_mobile,receiver_email):
        # 乘客姓名
    ...
    def start_brush(self):
        ...
        try:
            ...
            self.driver.cookies.add({'_jd_save_fromDate':self.from_time})
            self.driver.reload()
            count=0
                        ...
                        if (current_time_value>=self.my_start_time) and (current_time_value<=self.my_end_time):
                            ...
                            if current_tr:
                                ...
                                if current_tr.find_by_tag('td')[self.seat_type_index].text=='--':
                                    ...
                                    # 有票，尝试预定
                                    print(car_no+'('+current_time+')刷到票了 （余票数:'+str(current_tr.find_by_tag('td')[self.seat_type_index].text)+'),开始尝试预定...')
                                    ...
                                    for p in self.passengers:
                                        ...
                                        if self.seat_type_value!=0:
                                            self.driver.find_by_xpath("//select[@id='seatType_"+str(key_value)+"']/option[@value='"+str(self.seat_type_value)+"']").first.click()
                                        key_value+=1
                                        time.sleep(0.2)
                                        if p[-1]==')':
                                    ...
                                    self.send_sms(self.receiver_mobile,'您的验证码是：8888。请不要把验证码泄漏给其他人')
                            else:
                                print('不存在车次,已结束当前刷票，请重新开启！')
                                sys.exit(1)
                        elif current_time_value>self.my_start_time:
                            break
    ...
    def send_sms(self,mobile,sms_info):
        ...
        account='C59782899'
        password='19d4d9c0796532c7328e8b82e2812655'
        params=parse.urlencode(
            {'account':account,'password':password,'content':sms_info,'mobile':mobile,'format':"json"}
        )
...
def get_city():
    ...
    for s in new_station.keys():
        str=ascii(s)
        a=str.split('\\')
        b=('%'+'%'.join(a[1:])+'%2C').replace('\'','')+new_station[s]
        new_station[s]=b
    return new_station
...
if __name__ == '__main__':
    ...
    city_list=get_city()
    # 出发站
    from_input=input('请输入出发站，只需要输入首字母就行（例如"北京"）：')
    ...
    # 开始抢票
    # passengers=['张三']
    # from_time='2020-04-01'
    # from_station='%u5317%u4EAC%2CBJP'
    # to_station='%u592A%u539F%2CTYV'
    # my_start='09:00'
    # my_end='20:00'
    # seat_type='硬座'
    # receiver_mobile='18888888888'
    # receiver_email='test@163.com'
